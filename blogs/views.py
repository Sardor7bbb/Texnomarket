...
from blogs.form import BlogsCommentModelForm
from blogs.models import BlogModel, BlogTagModel, BlogCommentModel
...
class BlogDetailView(DetailView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        context["author"] = TeamModel.objects.all(),
        context["blog"] = BlogModel.objects.get(pk=self.kwargs['pk']),
        context["tags"] = BlogTagModel.objects.all(),
        context["famous_blogs"] = BlogModel.objects.all().order_by('-created_at')[:3],
        context["related_blogs"] = BlogModel.objects.filter(category__in=self.object.category.all())[:3],
        context['comments'] = BlogCommentModel.objects.filter(blogs=blogs),
